...
function displayTitle() {
	...
	firebase.auth().onAuthStateChanged(function(user) {
			...
			planRef.on("value", function(snapshot) {
				...
				snapshot.forEach(function(child) {
					if (child.val().PlanName == userPlanName) {
						//retrieve the corresponding Medication Plan Name
						MedicationName = child.val().Medication;
						//console.log(child.val().MedicationPlan);
						pageTitle.innerHTML = child.val().Medication;
		...
		//FOR DISPLAYING CONTENTS
			//find the corresponding Medication node 
			var edRef = firebase.database().ref("Medication");
			edRef.on("value", function(snapshot) {
				snapshot.forEach(function(child) {
					if (child.val().Page == MedicationName) {
